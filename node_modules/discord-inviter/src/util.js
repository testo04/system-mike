...
class inviteTracker extends Event {
  ...
  static async getInfo(client, code) {
    ...
    if (!code) return error("Invite url/code required");
    try {
      var invite = await client.fetchInvite(code);
  ...
  async resolveInvites(guild, invites) {
    try {
      ...
      if (guild.features.includes("VANITY_URL") || guild.features.includes("VanityURL")) {
    ...
    try {
      if (member.user.bot) {
        ...
        if (findInvite) {
          ...
          !findInvite &&
          (member.guild.features.includes("VANITY_URL") || member.guild.features.includes("VanityURL"))
        ) {
